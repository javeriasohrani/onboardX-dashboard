...
export default function NumberModal({ key, data, handleSubmit, number, handleClose }) {


    const [old, setOld] = useState(data);
    const [numberselect, setNumberSelect] = useState(70);
    const handleNumbertSelect = (event) => {
        setNumberSelect(event.target.value);
    };
...
console.log(old)
    return (
        <Modal open={number} onClose={handleClose}>
            <Box sx={style}>
                <IconButton
                    aria-label="close"
                    onClick={handleClose}
                ...
                <TextField hiddenLabel name="number" onChange={handleNumberChange} placeholder="Enter Field Name " id="margin-normal" margin="normal" />
                <Heading>Placeholder</Heading>
                <TextField hiddenLabel name="number" onChange={handleNumberChange} placeholder="Enter Your Number" id="margin-normal" margin="normal" />
                <Heading sx={{ marginBottom: '5px' }}>Field Type</Heading>
                <Select
                    value={numberselect}
                    onChange={handleNumbertSelect}
                    displayEmpty
                    sx={{ width: '100%' }}
                >
