...
import { All_Users, createNewUser } from "../Apis";
...
function NewUserModal({ open, handleClose }) {
  ...
  const handleSubmitPrevent = (event) => {
  ...
  const submitUserData = async () => {
    ...
    setUserItems(res.data.rows);
    setUser(() => ({}))
    res.status === 201 && getUsers();
          ...
          <Grid container>
            <Grid item sm={12} md={6} sx={{ p: "10px" }}>
              ...
              <List
                ...
                sx={{ height: "300px", width: "100%", overflow: "auto" }}
              >
                {user.map((emp) => (
                  <Box>
                    <ListItem
            ...
            <Grid item sm={12} md={5}>
              <Box sx={{ ml: "18%" }}>
                ...
                <Button
                  variant="contained"
                  sx={{ width: "135px", height: "40px", mt: "20px" }}
                  onClick={() => {
                    // handleClose();
                    submitUserData();
                    // handleSubmitPrevent()
