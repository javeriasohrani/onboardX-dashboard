...
function EmpTable() {
    ...
    const authAxios = axios.create({
        baseURL: 'https://tvrj97vxf0.execute-api.us-east-1.amazonaws.com/dev/',
        headers: {
    ...
    const getEmployees = async () => {
        setIsLoading(true);
        const res = await axios.get('https://tvrj97vxf0.execute-api.us-east-1.amazonaws.com/dev/all-employees')
            .then((res) => {
    ...
    const DeleteEmp = (deleteID) => {
        // alert(deleteID)
        const res = authAxios.delete(`all-employees/${deleteID}`)
            .then(responce => {
        ...
        <Box sx={{ bgcolor: '#F4F5F7', overflow: { md: 'hidden', xs: "auto" } }}>
            ...
            <Grid container sx={{ height: '40%', p: '20px' }}>
                ...
                <Grid item xs={12} md={12} sx={{ paddingBottom: '15px' }}>
                    <FormControl sx={{ width: '250px', backgroundColor: "#ffffff" }}>
                        ...
                        >
                            ...
                            {employees.map(item => (
                                <MenuItem value={item.employee_id}>{item.employee_name}</MenuItem>
                            ))}
                ...
                <Grid item xs={12} md={12} sx={{ height: '70vh', }}>
                            ...
                            <TableContainer className={classes.root} component={Box} sx={{ height: '100%', width: '100%' }}>
                                <Table stickyHeader={true} size="small">
                                    <TableHead>
                                        <TableRow>
                                            <TableCell>
                                                <Text sx={{ fontSize: '14px' }}> Employee Name</Text></TableCell>
                                            <TableCell align='center'><Text sx={{ fontSize: '14px' }}>Department</Text></TableCell>
                                            <TableCell align='right'><Text sx={{ fontSize: '14px' }}>Line Manager</Text></TableCell>
                                            <TableCell></TableCell>
                                        </TableRow>
                                    </TableHead>
                                    <TableBody>
                                                    ...
                                                    <Box>
                                                        <Link to={`/user-profile/${data.employee_id}`}>
                                                            <IconButton edge="end" aria-label="delete">
                                                                <BorderColorOutlinedIcon sx={{ color: '#2f49a1', paddingRight: '20px' }} />
                                                            </IconButton>
                                                        </Link>
                                                        <IconButton edge="end" aria-label="delete" onClick={() => DeleteEmp(data.employee_id)}>
                    ...
                    <Fab onClick={handleEmployee} color="primary" aria-label="add" sx={{ position: 'absolute', bottom: { md: 35, xs: 5 }, right: { md: 35, xs: 0 } }}>
                        <AddIcon />
                    </Fab>
                </Grid>
            </Grid>
            <RegisterEmpModal open={addemp} handleClose={handleEmployee} />
